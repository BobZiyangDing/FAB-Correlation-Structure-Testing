...
###### generate data ######
###########################
gen_psd_mtx <- function(p, k=1, zero_ratio = 0.05){
  W = matrix(runif(p*k),nrow=p, ncol=k)
  ratio_col <- zero_ratio ^ ( 1/k )
  for(i in 1:k)
  {
    zero_idx <- sample.int( p, p*ratio_col )
    W[ zero_idx , i] = 0
  }
  CovMat <- W %*% t(W) + eye(p) 
...
gen_sim_data <- function(n, ref_n, p, num_in_group, k, zero_ratio){
  ttl <- p*(p-1)/2
  ...
  while(!is.positive.definite(Sigma)){
    ...
    Sigma <- gen_psd_mtx(p, k, zero_ratio)
  ...
  print(paste("There are ", num_null, "nulls; and ", dim(alter_idx_cbind)[1], " non-nulls", sep = " ") )
...
{
  data_info_sim_boot <- gen_sim_data(n=n, ref_n=ref_n, p=p, num_in_group=num_in_group, k=50, zero_ratio = 0.002)
...
makeRejPlot <- function(algoResults, have_Truth = TRUE){
  ...
  UMPU_rej <- ifelse(UMPU_ps<0.01, "REJ", "NULL")
  FAB_rej <- ifelse(FAB_ps<0.01, "REJ", "NULL")
  
  
  Difference <- ifelse(FAB_rej == UMPU_rej, 
                       "Identical", 
                      ifelse( FAB_rej == "REJ", "FAB Additional Rej", "UMPU Additional Rej"))
  melted_rej_diff <- melt(Difference)
  ...
  rej_diff <- ggplot(data = melted_rej_diff, aes(x=Var1, y=Var2, fill=value)) + 
    geom_tile() + ggtitle("Rejection Difference") +
    theme_light()+
    theme(plot.title = element_text(hjust = 0.5), legend.position="bottom") +
    xlab("x") + ylab("y") + 
...
makePDistPlot <- function(algoResults, have_Truth = TRUE){
  ...
  if(have_Truth)
  {
    ...
    print(sum(FAB_alter < 0.05) - sum(UMPU_alter < 0.05) )
...
massiveConfusionExp <- function(n, p, num_in_group, times, method){
  UMPU_alpha_s <- c()
  UMPU_power_s <- c()
  UMPU_typeII_s <- c()
  UMPU_typeI_s <- c()
  ...
  for( i in 1:times){
    data_info <- gen_sim_data(n=n, ref_n=n, p=p, num_in_group=num_in_group, k=50, zero_ratio =0.002)
    ...
    if(method == "boot"){
      ...
      # algoResults <- FAB_general_boot(data_info$data_raw, 
      #                                 data_info$data_ref, 
      #                                 num_in_group = 5, 
      #                                 shrinkage=0 , 
      #                                 need_UMPU = TRUE, 
